...
class Trainer():
    ...
    def __init__(self, opt, logger):
        ...
        self.opt = opt #将传入的opt参数赋值给self.opt，opt包含了训练的各种选项。
        self.opt.isTrain = True
        self.logger = logger#用于记录训练过程中的日志。
        self.visualizer = Visualizer(opt)#使用opt初始化一个可视化器Visualizer，用于可视化训练过程中的数据。
        self.device = torch.device(f'cuda:{opt.gpu_ids[0]}') if torch.cuda.is_available() else 'cpu'

        self.epochs = opt.n_epochs#表示训练的总轮数。
        self.start_epochs = opt.start_epoch#表示训练的起始轮数。
        self.train_batch_size = self.opt.train_batch_size#表示训练时的批大小。
        self.temperature = self.opt.temperature#表示蒸馏过程中的温度参数。

        dataLoader = create_dataLoader(opt)#创建一个数据加载器dataLoader。
        self.trainLoader = dataLoader.trainLoader#训练数据加载器赋值给self.trainLoader。
        self.testLoader = dataLoader.testLoader#测试数据加载器赋值给self.testLoader。

        class_weights = torch.tensor([0.4, 0.6])
        self.criterion_CE = nn.CrossEntropyLoss(weight=class_weights).to(self.device)#创建一个交叉熵损失函数criterion_CE，并将其移动到对应的设备上。



        self.criterion_KL = nn.KLDivLoss(reduction='batchmean').to(self.device)#创建一个KL散度损失函数criterion_KL，并将其移动到对应的设备上。

        self.model_num = opt.model_num#表示模型的数量。
        self.models = []
        self.optimizers = []
        self.schedulers = []
        for i in range(self.model_num):#循环model_num次：
            model = create_model(opt).to(self.device)#使用opt创建一个模型model，并将其移动到对应的设备上。
            optimizer = optim.SGD(model.parameters(), lr=opt.lr, momentum=opt.momentum,
                                  weight_decay=opt.weight_decay,
                                  nesterov=True)#创建一个SGD优化器optimizer，用于优化模型参数。
            scheduler = utils.get_scheduler(optimizer, opt)#创建一个学习率调度器scheduler。
    ...
    def init_self_ditsllation_models(self):#初始化自蒸馏模型
        ...
        if str.startswith(self.opt.dataset, 'cifar'):#根据数据集类型确定输入尺寸，对于cifar数据集，输入尺寸为(1, 3, 32, 32)，对于其他数据集，输入尺寸为(1, 3, 224, 224)。
