...
class DWCGhost_SingleResidual(nn.Module):
    """
    单残差连接消融实验模型
    保留第一个残差连接，移除第二个残差连接
    """
    def __init__(self, num_classes=2, leader=False, trans_fusion_info=None):
        super(DWCGhost_SingleResidual, self).__init__()
        ...
        # 注意：match_channel_3 仍然需要，因为第二个残差连接的代码虽然被注释，但可能在其他地方使用
        # 但为了保持代码一致性，我们保留它但不使用
        ...
        # 注意：cross_attention_2 仍然定义，但不会被使用（因为第二个残差连接被移除）
    ...
    def forward(self, x):
        ...
        # ========== 第二个残差连接（移除） ==========
        # 注意：这里不再保存 x2，也不进行第二个残差连接
        # x2 = x  # 注释掉：不再保存用于第二个残差连接
        x = self.features[1](x)  # 直接通过第二个GhostBottleneck，不进行残差连接
        x = self.match_channel_2(x)
        x = self.self_attention_2(x)
        
        # 以下代码被注释掉，因为不再需要第二个残差连接
        ...
        # x += x2_downsampled  # 注释掉：第二个残差连接被移除
