...
# 设置日志记录
log_filename = os.path.join(log_dir, f'ShuffleNet_g1_training1_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
...
model = ShuffleNet(groups=4, num_layers=[4, 8, 4], num_channels=[144, 288, 576], num_classes=2).to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=0.0001)
criterion = nn.CrossEntropyLoss()

# 保存初始参数
logging.info(f'Model: ShuffleNet_g1')
...
num_epochs = 96
best_accuracy = 0.0
best_model_path = 'ShuffleNet_g1_best_model1.pth'
